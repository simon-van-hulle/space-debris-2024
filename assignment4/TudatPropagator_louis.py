...
# Original propagation for one timepoint
def propagate_state_and_covar_single_original(Xo, Po, tvec, state_params, int_params, bodies=None, alpha=1e-4):
    '''
    This function propagates an orbit using physical parameters provided in
...
# Original propagation for full orbit
def propagate_state_and_covar_original(Xo, Po, tvec, state_params, int_params, bodies=None, alpha=1e-4):
    ...
    return tf_array, states_array, Pf_array

# Backward propagation
def propagate_state_and_covar_backward(Xo, Po, tvec, state_params, int_params, bodies=None, alpha=1e-4):
    '''
    This function propagates an orbit using physical parameters provided in
    state_params and integration parameters provided in int_params. It also
    propagates the Gaussian covariance matrix using the Unscented transform.
    ...
    # Create termination settings
    forward_termination_condition = propagation_setup.propagator.time_termination(
        simulation_start_epoch, terminate_exactly_on_final_condition=True
    )
    backward_termination_condition = propagation_setup.propagator.time_termination(
        simulation_end_epoch, terminate_exactly_on_final_condition=True
    )
    termination_condition = propagation_setup.propagator.non_sequential_termination(
        forward_termination_condition, backward_termination_condition
    )
