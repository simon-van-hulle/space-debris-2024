...
def propagate_orbit_mod_output(Xo, tvec, state_params, int_params, bodies=None):
    '''
    this function is the same as propagate_orbit, but it returns only the states that are needed to compute the rms:
    the observations are not a single batch but rather 7 batches of 10 minutes each. thus only the relevant states are
    given in output. this is done performing a key check with the epochs in tvec[1:]. the first one is discarded bc it's
    ...
    if 'Earth' in bodies_to_create:
        ...
        if sph_deg == 0 and sph_ord == 0:
            ...
            acceleration_settings_setup['Earth'] = [
                propagation_setup.acceleration.spherical_harmonic_gravity(sph_deg, sph_ord)]
    ...
    # Extract the resulting state history and keep only the relevant timesteps, then convert to array
    states = dynamics_simulator.state_history
    states_copy = states.copy()
    for epoch in states.keys():
        if epoch not in (tvec[1:]):
            del states_copy[epoch]

    states_array = result2array(states_copy)
...
def propagate_state_and_covar_original(Xo, Po, tvec, state_params, int_params, bodies=None, alpha=1e-4):
    '''
    This function propagates an orbit using physical parameters provided in
...
def propagate_orbit_and_get_dep_var(Xo, tvec, state_params, int_params, bodies=None):
    '''
    This function propagates an orbit using physical parameters provided in
    state_params and integration parameters provided in int_params.
    ...
    for jj in range(N):
        ...
        dependent_variables_to_save = [
            propagation_setup.dependent_variable.single_acceleration(propagation_setup.acceleration.cannonball_radiation_pressure_type, jj_str, 'Sun'),
            propagation_setup.dependent_variable.relative_position('Sun', jj_str),
        ]
    ...
    return tout, Xout, dependent_variables